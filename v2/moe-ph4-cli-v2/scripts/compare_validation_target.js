...
function usage() {
  console.log(`Usage:
  node compare_validation_target.js --candidate-csv curated_features.csv --target-json validation_target.json --out-dir report_dir
  node compare_validation_target.js --self-test

Purpose:
  Compare a curated pharmacophore feature set against an validation target model.
...
function mdReport(result) {
  const lines = [];
  lines.push(`# Pharmacophore Validation-Target Comparison`);
  lines.push("");
  ...
  lines.push("");
  lines.push("Interpretation: strict requires matching family and near-identical coordinates; region allows the same mechanism zone to be recovered with validation-level tolerance.");
  return lines.join("\n");
}

function writeOutputs(result, outDir) {
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, "validation_target_comparison.json"), JSON.stringify(result, null, 2), "utf8");
  fs.writeFileSync(path.join(outDir, "validation_target_comparison.md"), mdReport(result), "utf8");
}
...
function main() {
  ...
  console.log(`Wrote ${path.join(args.out_dir, "validation_target_comparison.md")}`);
}
