...
公开论文里的药效团往往不是“真值坐标文件”，而是图、表、RMSD、GH score、命中率、对接 pose 和人工解释的组合。2.0 的比较层把这些信息转成可审计的 `article_target.json`，用于判断模型是否学到客观规律。
...
- `selected strict`：最终 selected 模型中，有多少文章核心点在同家族和近坐标下被命中。它衡量“是否复现文章点位”。
- `selected region`：最终 selected 模型中，有多少文章核心机制区域被命中。它衡量“是否找到同一个功能热点”。
